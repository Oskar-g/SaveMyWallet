...
--
DELIMITER $$
CREATE TRIGGER `ins_concept` BEFORE INSERT ON `movements` FOR EACH ROW INSERT IGNORE INTO concepts values(NEW.concept)
$$
DELIMITER ;


DELIMITER $$
CREATE TRIGGER `upd_concept` BEFORE update ON `movements` FOR EACH ROW INSERT IGNORE INTO concepts values(NEW.concept)
$$
DELIMITER ;
